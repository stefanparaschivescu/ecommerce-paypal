...
import { addOrder, fetchOrderById, fetchOrderByPaymentId, fetchOrderItemsByOrderId } from "./order-thunks";
...
export const orderSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(fetchOrderByPaymentId.pending, (state) => {
            state.loadingState = LoadingStatus.LOADING;
        });
        builder.addCase(fetchOrderByPaymentId.fulfilled, (state, action) => {
            state.order = action.payload;
            state.loadingState = LoadingStatus.LOADED;
        });
